...
router.post('/', async (req, res) => {
  try {
    ...
    // 驗證必要欄位
    if (!project || !system || !projectManager || !startDate || !expectedEndDate) {
      return res.status(400).json({ 
        error: 'Required fields: project, system, projectManager, startDate, expectedEndDate' 
      });
    }
  ...
  try {
    ...
    await db.run(
      ...
      [
        ...
        updates.startDate || existingProject.start_date,
        updates.expectedEndDate || existingProject.expected_end_date,
